...
import { VisitorRequestStatus, VisitorRequestFilter } from '../types/request.types';
import { Search } from 'lucide-react';
import { Input } from '@/components/ui/input';

interface RequestFiltersProps {
  onFilterChange: (filters: Partial<VisitorRequestFilter>) => void;
}
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 font-poppins bg-white p-4 rounded-xl border shadow-sm">
      ...
      >
        ...
        <option value={VisitorRequestStatus.PENDING}>Pending</option>
        <option value={VisitorRequestStatus.APPROVED}>Approved</option>
        <option value={VisitorRequestStatus.REJECTED}>Rejected</option>
        <option value={VisitorRequestStatus.CANCELLED}>Cancelled</option>
      </select>
