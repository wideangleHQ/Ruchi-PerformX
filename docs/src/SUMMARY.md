# Summary

[Start here](p1_overview.md)

# Phase 1 (shipped)

- [Local setup](p1_setup.md)
- [Architecture](p1_architecture.md)
- [Data model](p1_data_model.md)
- [Auth and roles](p1_auth_and_roles.md)
- [Tasks](p1_tasks.md)
- [Self actions](p1_self_actions.md)
- [Requests and transfers](p1_requests_and_transfers.md)
- [Notifications and realtime](p1_notifications.md)
- [Scoring](p1_scoring.md)
- [Visitor management](p1_vms.md)
- [API reference](p1_api_reference.md)
- [Code conventions](p1_conventions.md)
- [Known gaps and dead code](p1_known_gaps.md)

# Phase 2 (to build)

- [Plan and sequencing](p2_plan.md)
- [Schema changes](p2_data_model.md)
- [Leave management](p2_leave.md)
- [Projects](p2_projects.md)
- [R&D and company assets](p2_rnd_and_assets.md)
- [Home dashboard: birthdays, holidays, polls](p2_dashboard_social.md)
- [Vendor management](p2_vendors.md)
- [Notification engine rebuild](p2_notifications.md)
- [CareerX and VMS integration](p2_integration.md)
